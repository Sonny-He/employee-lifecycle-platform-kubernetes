...
if (-not (Test-Path "employee-portal/db/init-employee-db.sql")) {
    Write-Host "? employee-portal/db/init-employee-db.sql not found" -ForegroundColor Red
    exit 1
}

Write-Host "  Copying SQL file to postgres pod..." -ForegroundColor Gray
kubectl cp employee-portal/db/init-employee-db.sql employee-services/postgres-0:/tmp/employee-portal/db/init-employee-db.sql 2>&1 | Out-Null
if ($LASTEXITCODE -ne 0) {
...
kubectl exec -n employee-services postgres-0 -- psql -U admin -d employees -f /tmp/employee-portal/db/init-employee-db.sql 2>&1 | ForEach-Object {
    $line = $_.ToString()
    if ($line -match "ERROR:") {
